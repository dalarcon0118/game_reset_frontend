...
export const NumericKeyboard: React.FC<NumericKeyboardProps> = ({
    ...
    const formatInput = (input: string) => {
        if (annotationType === 'amount') return input;
        
        // Format bets as (15)-(25)
        ...
        <View style={styles.keyboard}>
            <View style={[styles.displayContainer, { backgroundColor: theme['background-basic-color-3'] }]}>
                <Text category="h5" style={styles.displayText}>
                    {formatInput(currentInput) || (annotationType === 'bet' ? 'Introducir Apuestas' : 'Introducir Monto')}
                </Text>
            </View>
            {rows.map((row, idx) => (
...
export { BetNumericKeyboard } from './BetNumericKeyboard';
export { AmountNumericKeyboard } from './AmountNumericKeyboard';
