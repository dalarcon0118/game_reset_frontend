...
import { Text, Card, Spinner, Layout, useTheme, Button } from '@ui-kitten/components';
import { SafeAreaView } from 'react-native-safe-area-context';
import { router } from 'expo-router';
import { ArrowLeft, Trophy, Hash, Award, Layers } from 'lucide-react-native';

import useDataFetch from '@/shared/hooks/useDataFetch';
import { WinningService } from '@/shared/services/winning';
import { DrawType, WinningRecord } from '@/types';
...
export default function RewardsScreen({ drawId }: RewardsScreenProps) {
    ...
    const [fetchWinningNumbers, winningNumbers, isLoadingNumbers, errorNumbers] = useDataFetch<WinningRecord | null, [string]>(WinningService.getWinningNumber)


    useEffect(() => {
        fetchWinningNumbers(drawId)
    }, [drawId, fetchWinningNumbers]);
        ...
        return (
            <View style={styles.errorContainer}>
                ...
                <Text appearance="hint" style={{ marginTop: 8, textAlign: 'center' }}>
                    {errorNumbers instanceof ApiClientError
                        ? errorNumbers.data.detail || errorNumbers.message
                ...
                <Button
                    ...
                    appearance="outline"
                    status="primary"
                    onPress={() => fetchWinningNumbers(drawId)}
                >
                    Reintentar
                </Button>
                <Button
                    ...
                    <>
                        ...
                        <Card style={styles.mainCard} status="primary">
                            ...
                            <Text appearance="hint" style={styles.dateText}>
                                {new Date(winningNumbers!.date).toLocaleDateString('es-ES', {
                                    weekday: 'long',
                                    year: 'numeric',
                                    month: 'long',
                                    day: 'numeric'
