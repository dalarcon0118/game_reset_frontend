...
import { mapIncidentToBankerReport } from './utils';
import { logger } from '@/shared/utils/logger';

const log = logger.withTag('BANKER_REPORTS_LIST');

export default function BankerReportsListScreen() {
    ...
    const handleUpdateStatus = async (id: string, status: string, notes: string) => {
        ...
        try {
            ...
            log.error('Error updating status in screen', { error: err, incidentId: id, status: backendStatus });
            throw err;
        }
    };
    ...
    const reports = incidents ? incidents.map(mapIncidentToBankerReport) : [];
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            <Flex vertical flex={1}>
                ...
                <Flex align="center" justify="between" padding={spacing.lg} style={styles.header}>
                    <Label type="title" value="Reportes Bancarios" />
                    <Flex gap={spacing.sm}>
                        <TouchableOpacity
