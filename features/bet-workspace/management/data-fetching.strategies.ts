...
import { ListActions } from './management.utils';
import { ManagementMsgType } from './management.types';
import { CoreMsgType } from '@/features/bet-workspace/core/msg';
import { RewardsRulesMsgType } from '@/features/bet-workspace/rewards/rewards.types';

// Define CommandDescriptor if not imported (as it seems to be missing in imports)
type CommandDescriptor = any;
...
export function buildCommandsForMode(mode: ViewMode, drawId: string): CommandDescriptor[] {
  ...
  if (config.fetchDrawInfo) {
    ...
    commands.push(drawInfoCmd as CommandDescriptor);
  }
  ...
  if (config.fetchBetTypes) {
    ...
    commands.push(betTypesCmd as CommandDescriptor);
  }
